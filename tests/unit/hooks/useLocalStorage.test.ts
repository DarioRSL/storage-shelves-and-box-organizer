...
describe("useLocalStorage", () => {
  ...
  describe("onChange callback", () => {
    ...
    it("should call onChange with functional update result", () => {
      const onChangeMock = vi.fn();
      const { result } = renderHook(() => useLocalStorage<number>("callback-counter", 10, { onChange: onChangeMock }));
  ...
  describe("Real-world use cases", () => {
    ...
    it("should handle user preferences with onChange", () => {
      ...
      const { result } = renderHook(() =>
        useLocalStorage("preferences", { fontSize: 16, language: "en" }, { onChange: applyPreferences })
      );
    ...
    it("should handle form draft autosave", () => {
      const { result } = renderHook(() => useLocalStorage("form-draft", { title: "", content: "" }));
  ...
  describe("Edge cases", () => {
    ...
    it("should handle setting same value multiple times", () => {
      const onChangeMock = vi.fn();
      const { result } = renderHook(() => useLocalStorage("same-value", "value", { onChange: onChangeMock }));
