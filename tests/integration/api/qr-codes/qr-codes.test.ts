...
describe.skip("GET /api/qr-codes", () => {
  ...
  it("should list all QR codes for workspace", async () => {
    ...
    const response = await authenticatedGet(
      `/api/qr-codes?workspace_id=${dataset.workspaces.primary.id}`,
  ...
  it("should filter by status", async () => {
    const dataset = await seedInitialDataset();
    const response = await authenticatedGet(
      `/api/qr-codes?workspace_id=${dataset.workspaces.primary.id}&status=generated`,
      dataset.users.admin.token
    );
    assertSuccess(response);
  });
...
describe.skip("POST /api/qr-codes/batch", () => {
  ...
  it("should generate batch of 20 QR codes", async () => {
    const dataset = await seedInitialDataset();
    const response = await authenticatedPost("/api/qr-codes/batch", dataset.users.admin.token, {
      workspace_id: dataset.workspaces.primary.id,
      count: 20,
    });
    assertSuccess(response);
  ...
  it("should reject without authentication", async () => {
    const response = await authenticatedPost("/api/qr-codes/batch", "", {
  ...
  it("should reject from non-member", async () => {
    const dataset = await seedInitialDataset();
    const outsider = await createAuthenticatedUser({
      email: "batch-out@example.com",
      password: "SecurePass123!",
      full_name: "Batch Out",
    });
    const response = await authenticatedPost("/api/qr-codes/batch", outsider.token, {
      workspace_id: dataset.workspaces.primary.id,
      count: 10,
    });
    assertError(response, 403);
  ...
  it("should reject non-existent workspace", async () => {
    const testUser = await createAuthenticatedUser({
      email: "qr-404@example.com",
      password: "SecurePass123!",
      full_name: "QR 404",
    });
    const response = await authenticatedPost("/api/qr-codes/batch", testUser.token, {
