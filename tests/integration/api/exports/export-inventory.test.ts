...
describe.skip('GET /api/export/inventory', () => {
  ...
  it('should export all boxes as CSV', async () => {
    ...
    const response = await authenticatedGet(`/api/export/inventory?workspace_id=${dataset.workspaces.primary.id}`, dataset.users.admin.token);
  ...
  it.skip('should filter by location_id', async () => {
    ...
    const response = await authenticatedGet(`/api/export/inventory?workspace_id=${dataset.workspaces.primary.id}&location_id=${location.id}`, dataset.users.admin.token);
    assertSuccess(response);
  ...
  it('should include name, description, tags, location, QR, status, short_id', async () => {
    const dataset = await seedInitialDataset();
    const adminClient = getAdminSupabaseClient();
    const [location] = await adminClient.from('locations').insert(createRootLocationFixture(dataset.workspaces.primary.id, 'TestLoc')).select().throwOnError();
    const boxData = { ...createBoxFixture(dataset.workspaces.primary.id, location.id, 'TestBox'), description: 'Test desc', tags: ['tag1', 'tag2'] };
  ...
  it('should handle special characters in CSV (quotes, commas, newlines)', async () => {
    ...
    const adminClient = getAdminSupabaseClient();
    const [location] = await adminClient.from('locations').insert(createRootLocationFixture(dataset.workspaces.primary.id, 'Loc')).select().throwOnError();
    const boxData = { ...createBoxFixture(dataset.workspaces.primary.id, location.id, 'Box, with "quotes"'), description: 'Line1\nLine2' };
    await seedTable('boxes', [boxData]);
    const response = await authenticatedGet(`/api/export/inventory?workspace_id=${dataset.workspaces.primary.id}`, dataset.users.admin.token);
    assertSuccess(response);
  ...
  it('should handle empty workspace', async () => {
    const testUser = await createAuthenticatedUser({ email: 'export-empty@example.com', password: 'SecurePass123!', full_name: 'Export Empty' });
    const adminClient = getAdminSupabaseClient();
    const [workspace] = await adminClient.from('workspaces').insert({ name: 'Empty WS', owner_id: testUser.id }).select().throwOnError();
    await adminClient.from('workspace_members').insert({ workspace_id: workspace.id, user_id: testUser.id, role: 'owner' }).throwOnError();
  ...
  it('should reject invalid location_id format', async () => {
    const dataset = await seedInitialDataset();
    const response = await authenticatedGet(`/api/export/inventory?workspace_id=${dataset.workspaces.primary.id}&location_id=not-uuid`, dataset.users.admin.token);
    assertError(response, 400);
  });
  ...
  it('should reject from non-member', async () => {
    const dataset = await seedInitialDataset();
    const outsider = await createAuthenticatedUser({ email: 'export-out@example.com', password: 'SecurePass123!', full_name: 'Export Out' });
    const response = await authenticatedGet(`/api/export/inventory?workspace_id=${dataset.workspaces.primary.id}`, outsider.token);
