...
describe.skip("GET /api/locations", () => {
  ...
  describe("Success Cases", () => {
    it("should list all locations for workspace", async () => {
      ...
      // Act: Get locations
      const response = await authenticatedGet(`/api/locations?workspace_id=${primaryWorkspaceId}`, adminUser.token);
    ...
    it.skip("should return empty array if workspace has no locations", async () => {
      ...
      const [workspace] = await adminClient.from("workspaces").insert(workspaceData).select().throwOnError();
      ...
      // Act
      const response = await authenticatedGet(`/api/locations?workspace_id=${workspace.id}`, testUser.token);
    ...
    it("should return only locations user has access to via RLS", async () => {
      ...
      const response = await authenticatedGet(`/api/locations?workspace_id=${secondaryWorkspaceId}`, viewerUser.token);
  ...
  describe.skip("Validation Errors (400)", () => {
    ...
    it("should reject request with invalid workspace_id format", async () => {
      ...
      const response = await authenticatedGet("/api/locations?workspace_id=not-a-uuid", adminUser.token);
  ...
  describe.skip("Authentication Errors (401)", () => {
    ...
    it("should reject request with invalid token", async () => {
      // Act
      const response = await authenticatedGet("/api/locations?workspace_id=123", "invalid.jwt.token");
  ...
  describe.skip("Authorization Errors (403)", () => {
    it("should reject request from non-member", async () => {
      ...
      const response = await authenticatedGet(`/api/locations?workspace_id=${primaryWorkspaceId}`, outsider.token);
...
describe.skip("POST /api/locations", () => {
  ...
  describe("Success Cases", () => {
    ...
    it.skip("should create location up to level 5 depth", async () => {
      ...
      const [level1] = await adminClient.from("locations").insert(level1Data).select().throwOnError();
