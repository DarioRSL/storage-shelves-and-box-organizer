...
describe.skip('GET /api/workspaces/:id', () => {
  ...
  describe.skip('Not Found Errors (404)', () => {
    it('should return 404 for non-existent workspace', async () => {
      ...
      // Act
      const response = await authenticatedGet(
        `/api/workspaces/${fakeWorkspaceId}`,
        testUser.token
      );
...
describe.skip('DELETE /api/workspaces/:id', () => {
  ...
  describe('Success Cases', () => {
    ...
    it('should cascade delete all workspace data', async () => {
      ...
      const { data: boxesBefore } = await adminClient
        .from('boxes')
        .select('id')
      ...
      const { data: workspaceAfter } = await adminClient
        .from('workspaces')
        .select('id')
        .eq('id', workspaceId)
        .single();
      ...
      const { data: boxesAfter } = await adminClient
        .from('boxes')
        .select('id')
        .eq('workspace_id', workspaceId);
    ...
    it('should not affect other workspaces', async () => {
      ...
      await authenticatedDelete(
        `/api/workspaces/${primaryWorkspaceId}`,
        adminUser.token
      );
