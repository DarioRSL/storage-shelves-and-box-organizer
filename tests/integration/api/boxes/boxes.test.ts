...
describe.skip("GET /api/boxes", () => {
  ...
  describe("Success Cases", () => {
    it("should list all boxes for workspace", async () => {
      ...
      // Act: Get boxes
      const response = await authenticatedGet(`/api/boxes?workspace_id=${primaryWorkspaceId}`, adminUser.token);
    ...
    it.skip("should return empty array if workspace has no boxes", async () => {
      ...
      // Act
      const response = await authenticatedGet(`/api/boxes?workspace_id=${workspace.id}`, testUser.token);
    ...
    it.skip("should return only boxes user has access to via RLS", async () => {
      ...
      const response = await authenticatedGet(`/api/boxes?workspace_id=${secondaryWorkspaceId}`, viewerUser.token);
  ...
  describe.skip("Authentication Errors (401)", () => {
    ...
    it.skip("should reject request with invalid token", async () => {
      // Act
      const response = await authenticatedGet("/api/boxes?workspace_id=123", "invalid.jwt.token");
  ...
  describe.skip("Authorization Errors (403)", () => {
    it("should reject request from non-member", async () => {
      ...
      const response = await authenticatedGet(`/api/boxes?workspace_id=${primaryWorkspaceId}`, outsider.token);
...
describe.skip("POST /api/boxes", () => {
  ...
  describe("Success Cases", () => {
    ...
    it.skip("should auto-generate search_vector via database trigger", async () => {
      ...
      const { data: box } = await adminClient.from("boxes").select("search_vector").eq("id", boxId).single();
    ...
    it.skip("should create box with QR code assignment", async () => {
      ...
      const { data: updatedQR } = await adminClient.from("qr_codes").select("status").eq("id", qrCode.id).single();
